    ...
    # · Standardised sampling ranges ----
      {
        mdl_data_std <- janzen %>% 
          ...
          mutate(
            sampling_min = sampling_max - settings["sampling_range"],
    ...
    # · Sampled species ----
      {
        mdl_data_buffered <- mdl_data_std %>% 
          ...
          group_by(location) %>% 
          mutate(
            max_buffer = sampling_mid + settings["buffer"],
            min_buffer = sampling_mid - settings["buffer"]
          ) %>% 
          rowwise() %>% 
    ...
    # · Model data ----
      {
        mdl_data <- mdl_data_buffered %>% 
          ...
          c(list(
            ...
            n_obs = length(mdl_data_buffered$elev_range),
