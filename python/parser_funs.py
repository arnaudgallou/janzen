...
def parse_elev(string, pattern):
    res = []
    pattern = regex.sub(r"\([^)]+\)$", r"\s?(?:m|ft)\\b", pattern)
...
def get_elev(string, pattern, parse_all = None, to_meter = None):
    if parse_all is None:
        ...
        elev = parse_elev(string, pattern)
        min_elev = min(elev)
        max_elev = max(elev)
...
def load_arguments():
  ...
  ap.add_argument(
    ...
    type = str,
    nargs = "+",
    help = "Minimum number of digit to use to detect elevational values. Can be useful to avoid matching unwanted numerical values such as trees height. By default, detects any numerical values of one digit or more."
  )
  # ap.add_argument("-l", "--level", type = str, help = "Should taxa at the genus level be extracted?)
  ap.add_argument(
    "-s", "--start",
    type = str,
  ...
  ap.add_argument(
    ...
    type = str,
    default = "m",
    help = "Unit of elevational values. Either 'm' (the default) or 'ft'. If --parse_elev is set, will automatically detect the unit and convert values to meter."
  )
  ap.add_argument(
    "-n", "--output_name",
    type = str,
...
def process_args():
    ...
    if args.case in ["uppercase", "U"]:
        patterns["head"] = regex.sub(r"{Ll}", "{Lu}", patterns["head"])
    if args.digit is not None:
        patterns["elev"] = regex.sub(r"d\K\+", "{" + regex.escape(args.digit) + ",}", patterns["elev"])
        patterns["tail"] = regex.sub(r"d\K", "{" + regex.escape(args.digit) + ",}", patterns["tail"])
    return args
