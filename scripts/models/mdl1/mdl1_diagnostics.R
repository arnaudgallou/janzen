####  Init  ####
  {
    ...
    # · Regressions ----
      {
        fit_sims <- rename(fit_sims, beta_rs = beta_rs1)
        fitted_draws <- slice_draws(fit_sims, beta_rs, n = 400)
        
        residuals <- tibble(
          alpha = mean(fitted_draws$alpha_rs),
          beta1 = mean(fitted_draws$beta_rs),
          beta2 = mean(fitted_draws$beta_rs2)
        )
        ...
        mdl_data %>% 
          ...
          ggplot(aes(
            x = dtr,
            y = elev_range - residuals$beta2 * sampling_range
          )) +
          geom_jitter(
            size = .1,
            width = .05,
          ...
          # ) +
          geom_abline(
            slope = residuals$beta1,
            intercept = residuals$alpha,
