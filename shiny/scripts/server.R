server <- function(input, output) {
  janzen_data <- reactive({
    data <- filter(
        shiny_janzen,
        singleton < input$singleton,
        sampling_size >= input$sampling_size
      )
    
    if ("standardise_sampling_size" %in% input$options) {
      data <- data %>% 
        mutate(
          sampling_min = sampling_max - input$sampling_size,
          sampling_size = sampling_max - sampling_min
        ) %>% 
        filter(elev_max > sampling_min) %>%
        mutate(
          elev_min = if_else(
            elev_min < sampling_min,
            sampling_min,
            elev_min
          ),
          elev_range = elev_max - elev_min
        )
    } else {
      data <- filter(data, sampling_min <= input$sampling_min)
    }
    
    if (all(c("show_locations", "show_sampling_size") %in% input$options)) {
      data <- mutate(data, label = paste0(location, " (", sampling_size, " m)"))
    } else if ("show_locations" %in% input$options) {
      data <- mutate(data, label = location)
    } else if ("show_sampling_size" %in% input$options) {
      data <- mutate(data, label = paste0(sampling_size, " m"))
    }
    
    if (input$species_by == "sigma") {
      data <- mutate(data, sigma = input$sigma)
    } else if (input$species_by == "mid_point") {
      data <- group_by(data, location) %>% 
        mutate(sigma = (sampling_max + sampling_min) / 2) %>% 
        ungroup()
    }
    
    if (any(c("sigma", "mid_point") %in% input$species_by)) {
      data <- data %>% 
        mutate(
          max_buffer = sigma + input$buffer,
          min_buffer = sigma - input$buffer
        ) %>%
        rowwise() %>%
        filter(
          any(between(c(elev_min, elev_max), min_buffer, max_buffer)) | elev_min < min_buffer & elev_max > max_buffer
        ) %>% 
        ungroup()
    }
    
    data <- left_join(data, shiny_bioclim, by = c("location", "elev_band")) %>% 
      group_by(location)
    
    if (input$species_by == "none") {
      data <- mutate(data, across(starts_with("bio"), mean))
    }
    
    data <- data %>% 
      mutate(elev_range = mean(elev_range)) %>% 
      ungroup() %>% 
      distinct(location, .keep_all = TRUE) %>%
      drop_na()
    
    # print(
    #   distinct(data, location, .keep_all = TRUE) %>% 
    #     select(location, sigma, sampling_min, sampling_max),
    #   n = 100
    # )
    data
  })
  
  output$janzen_plot <- renderPlot({
    bioclim_ls <- list(
      "Diurnal temperature range (°C)" = "bio2",
      "Temperature seasonality (°C)" = "bio4",
      "Mean annual temperature (°C)" = "bio1",
      "Mean annual precipitation (mm)" = "bio12",
      "Mean temp. of warmest quarter (°C)" = "bio10",
      "Mean temp. of coldest quarter (°C)" = "bio11"
    )
    
    plot <- janzen_data() %>% 
      ggplot(aes(
        x = .data[[input$bioclim]],
        y = elev_range
      )) +
      geom_point(size = 2) +
      theme_classic() +
      labs(
        y = "Species elevational range (m)",
        x = names(bioclim_ls[bioclim_ls == input$bioclim])
      ) +
      theme(
        axis.text = element_text(size = 14),
        axis.title = element_text(size = 17)
      )
    
    if (any(c("show_locations", "show_sampling_size") %in% input$options)) {
      plot <- plot + 
        geom_text_repel(aes(label = label, color = zone), size = 4.5, force = 5) +
        scale_color_manual(values = c("blue", "red"))
    }
    if ("lm" %in% input$options) {
      plot <- plot + geom_smooth(method = "lm")
    }
    
    plot + theme(legend.position = "none")
  })
}